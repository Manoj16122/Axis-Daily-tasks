        ...
        while(true){
            ...
            switch(choice){
                ...
                if(mainFile.BankObj.map.containsKey(accountNo1)){
                    Atm atmobj = new Atm();
                   mainFile.BankObj.map.replace(accountNo1, atmobj.checkBalance(mainFile.BankObj.map.get(accountNo1)));
                }
                break;
                case 4:
                System.out.println("Enter account No");
                ...
                if (mainFile.BankObj.map.containsKey(account_no)){
                    Atm atmobj = new Atm();
                    mainFile.BankObj.map.replace(account_no, atmobj.withdraw(mainFile.BankObj.map.get(account_no)));
                }
                break;
                case 5:
                System.out.println("Enter account no");
